...
class WindowsServicesTUI:
    ...
    def update_mssql_status_display(self, status_report):
        ...
        if mssql_data.get('deployed_crs'):
            self.status_walker.append(urwid.Text(('service_mssql', '☸️ DEPLOYED MSSQL CRs:')))
    ...
    def add_log_line(self, text):
        ...
        if 'ERROR' in text.upper() or '❌' in text:
            attr = 'log_error'
        elif 'WARNING' in text.upper() or 'WARN' in text.upper() or '⚠️' in text:
            attr = 'log_warning'
        else:
            attr = 'log_info'
            ...
            if service_key and service_key in status_report:
                ...
                if local_crs:
                    ...
                    if installable_crs:
                        ...
                        self.add_log_line("❌ No installable CRs found")
                        self.add_log_line("💡 All CRs are either deployed or disabled")
                else:
            ...
            if service_key and service_key in status_report:
                ...
                if deployed_crs:
                    ...
                    if uninstallable_crs:
                        ...
                        self.add_log_line("💡 Nothing to uninstall")
                else:
                    self.add_log_line(f"❌ No deployed {service_name} CRs found")
                    self.add_log_line("💡 Nothing to uninstall")
            else:
    ...
    def unhandled_input(self, key):
        ...
        self.add_log_line(f"🔍 GLOBAL KEY: {key}, menu_state: {self.menu_state}, has_popup: {hasattr(self, 'popup') and self.popup is not None}")
        ...
        if self.menu_state == 'service_popup':
            if key == 'escape':
                ...
                try:
                    ...
                    self.add_log_line(f"❌ Error getting selection: {e}")
                    return
    ...
    def show_ansible_install_options(self, service_type, status_report):
        ...
        self.add_log_line("  • Network configuration")
        self.add_log_line("💡 Ready for Ansible execution")
    ...
    def execute_cr_install(self, service_type, service_name, cr_name, cr_data):
        ...
        try:
            ...
            if not cr_data_copy:
                self.add_log_line(f"❌ Invalid CR file: {cr_file_path}")
                return
            ...
            try:
                ...
                if result.returncode == 0:
                    self.add_log_line(f"✅ Custom Resource applied successfully")
                    ...
                    self.add_log_line(f"🎭 Looking for playbook for CR kind: {cr_kind}")
                    
                    if playbook:
                        playbook_path = f"/root/kubernetes-installer/{playbook}"
                        self.add_log_line(f"🎭 Checking playbook path: {playbook_path}")
                        if os.path.exists(playbook_path):
                            self.add_log_line(f"🎭 Running Windows VM installation playbook...")
                            ...
                            if os.path.exists(inventory_path):
                                ...
                                self.add_log_line(f"📋 Using inventory: {inventory_path}")
                            else:
                                self.add_log_line(f"⚠️ No inventory file found, using localhost")
                            ...
                            self.add_log_line(f"🎭 Running: {' '.join(ansible_cmd)}")
                            
                            playbook_result = subprocess.run(ansible_cmd, 
                                                          capture_output=True, text=True)
                            
                            if playbook_result.returncode == 0:
                                self.add_log_line(f"✅ Windows VM installation completed successfully!")
                                self.add_log_line(f"🚀 VM {cr_name} should now be deployed and running")
                            else:
                                self.add_log_line(f"❌ Playbook failed: {playbook_result.stderr}")
                                if playbook_result.stdout:
                                    self.add_log_line(f"📋 Playbook output: {playbook_result.stdout[-200:]}")
                        else:
                            self.add_log_line(f"⚠️ Playbook not found: {playbook_path}")
                    else:
                        self.add_log_line(f"⚠️ No playbook mapping for CR kind: {cr_kind}")
        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    self.add_log_line(f"🗑️ Removing Custom Resource...")
                    ...
                    if result.returncode == 0:
                        self.add_log_line(f"✅ Custom Resource deleted")
                    else:
                        self.add_log_line(f"⚠️ CR deletion note: {result.stderr}")
    ...
    def execute_cr_apply(self, service_type, service_name, cr_name, cr_data):
        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    self.add_log_line(f"✅ Custom Resource applied successfully")
                    self.add_log_line(f"💡 CR {cr_name} is now deployed in the cluster")
                    ...
                    if cr_action == 'install':
                        ...
                        if playbook:
                            playbook_path = f"/root/kubernetes-installer/{playbook}"
                            if os.path.exists(playbook_path):
                                self.add_log_line(f"🎭 Running Windows VM installation playbook for {cr_action} action...")
                                ...
                                if playbook_result.returncode == 0:
                                    self.add_log_line(f"✅ Installation playbook completed successfully!")
                                    self.add_log_line(f"🚀 VM {cr_name} should now be running")
                                else:
                                    self.add_log_line(f"❌ Playbook failed: {playbook_result.stderr}")
                                    if playbook_result.stdout:
                                        self.add_log_line(f"📋 Playbook output: {playbook_result.stdout[-200:]}")
                            ...
                            self.add_log_line(f"⚠️ No playbook mapping for {cr_kind}")
                    else:
                        self.add_log_line(f"💡 CR applied with action '{cr_action}' - no playbook execution needed")
    ...
    def force_key_handler(self, key):
        ...
        self.add_log_line(f"🔑 FORCE_KEY: {key} - menu_state: {self.menu_state}")
        ...
        if has_popup and key == '1':
            self.add_log_line("🔥 FORCE: Number 1 pressed with popup open!")
            # Force close popup and trigger installation
            self.close_popup()
            self.add_log_line("🔥 FORCE: Calling execute_cr_install directly!")
        ...
        if has_popup and key == 'escape':
            self.add_log_line("🔥 FORCE: ESC pressed - closing popup!")
            self.close_popup()
            return
        
        # For CR popups, let Enter key pass through to urwid for proper button handling
        if self.menu_state == 'cr_popup' and key == 'enter':
            self.add_log_line("🔥 FORCE: Enter key in CR popup - letting urwid handle it!")
    ...
    def run(self):
        ...
        self.loop = urwid.MainLoop(
            self.main_frame,
            self.palette,
            unhandled_input=self.unhandled_input,  # Use normal handler for proper urwid behavior
            handle_mouse=True
        )
